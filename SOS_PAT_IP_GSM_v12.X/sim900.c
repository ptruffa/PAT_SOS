...
void sim900Handler(bool data_available){
    ...
    switch(state){
        ...
        case OK:
            ...
            moduleResonseBuffer[1] = NULL;
            moduleResonseBuffer[2] = NULL;
            moduleResonseBuffer[3] = NULL;
            if(cuenta_rings >= CANTIDAD_RINGS){ 
                state = ENLLAMADA;
                cuenta_rings = 0; 
            }else 
    ...
    if((maxTiempo >= DURACION_LLAMADA || button_pushed)&& f_llamada_en_curso){
        sim900Cortar();
        button_pushed = false;
        f_llamada_en_curso = false;
        maxTiempo = 0;
...
void sim900GetSMSnumber(){
    ...
    //printf("--%s--\r",numero_sms_recivido);
}
...
void sim900MarcarCentral(){
    ...
    sim900AudioGSM(ON);
    printf("numero:%d--cetral:%d--%d\r", numero, cantidad_centrales,numero%cantidad_centrales);
    delayms(100);
...
void sim900Bateria(){
    //delayms(100);
    if(VoltajeBateria < 10 && !f_bateriaBaja){
