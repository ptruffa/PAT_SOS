...
uint16_t secTicker = 1;
uint16_t secTickerLast = 1;
...
int main(void)
{
    ...
    sendCommand("AT+CREG?");
    sim900DummyRead();
    sendCommand("AT+CGDCONT=1,\"IP\",\"bam.entelpcs.cl\"");
    sim900DummyRead();
    sendCommand("AT+CPSI?");
    sim900DummyRead();
    sendCommand("AT+CGREG?");
    sim900DummyRead();
    //sendCommand("AT+CGSOCKCONT=1,\"IP\",\"CMNET\"");
    //sim900DummyRead();
    ...
    /*while(1){
        ...
        if(strncmp((const char*)moduleResonseBuffer, "+CMQTTSTART: 7", strlen("+CMQTTSTART: 7")) == 0)
            sim900DummyRead();
        else
            while(1);
        delaysegundos(2);
    ...
    while(1){
        sendCommand("AT+CMQTTSTART");
        sim900DummyRead();
        sendCommand("AT+CMQTTACCQ=0,\"client c\"");
        sim900DummyRead();
        sendCommand("AT+CMQTTWILLTOPIC=0,10");
        printf("will topic");
        UART1_Write(0x1A);
        UART1_Write(0x0D);
        sim900DummyRead();
        sendCommand("AT+CMQTTWILLMSG=0,56,1");
        printf("you?d better set will topic before setting will message");
        UART1_Write(0x1A);
        UART1_Write(0x0D);
        sim900DummyRead();
        sendCommand("AT+CMQTTCONNECT=0,\"tcp://patchile.dyndns.org:1883\",20,1,\"\",\"\"");
        sim900DummyRead();
        sendCommand("AT+CMQTTPUBTOPIC=0,13");
        printf("publish topic");
        UART1_Write(0x1A);
        UART1_Write(0x0D);
        sim900DummyRead();
        sendCommand("AT+CMQTTPUBMSG=0,62");
        printf("you?d better set publish topic before setting publish message.");
        UART1_Write(0x1A);
        UART1_Write(0x0D);
        sim900DummyRead();
